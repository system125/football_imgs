...
def shape_to_path(element):
    """Convert basic SVG shapes to <path d="..."> elements."""
    tag = element.tag.split("}")[-1]
    
    if tag == 'path':
        return element  # already a path
    ...
    # Create new <path> element with same style attributes
    path_el = ET.Element(f"{{{SVG_NS}}}path")
    path_el.set("d", d)
    for attr in element.attrib:
        if attr not in ['x', 'y', 'cx', 'cy', 'r', 'rx', 'ry', 'width', 'height', 'x1', 'y1', 'x2', 'y2', 'points']:
            path_el.set(attr, element.attrib[attr])
    return path_el
